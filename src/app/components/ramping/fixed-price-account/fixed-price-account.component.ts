...
import { FixedPriceAccountEditComponent } from '../fixed-price-account-edit/fixed-price-account-edit.component';
...
@Component({
  selector: 'app-fixed-price-account',
  templateUrl: './fixed-price-account.component.html',
  styleUrls: ['./fixed-price-account.component.scss']
})
export class FixedPriceAccountComponent implements AfterViewInit, OnInit,OnDestroy {
  ...
  getFixedMonthlyAccountDetails() {
    try {
      ...
      this._fixedMonthlyAccountService.getFixedMonthlyAccountDetails('Fixed Price').subscribe({
        next: (res:any) => {
  ...
  openEditDialog(element: any): void {
    try {
      const dialogRef = this.dialog.open(FixedPriceAccountEditComponent, {
        maxWidth: '95vw',
        minHeight: '300px',
        // height: '90%',
        width: '95%',
        panelClass: 'full-screen-modal',
        data: { element,  title: "Fixed Price Details" },
      });
      ...
      this.openValidationPopup(err, 'Error in opening modal !');
    }
  }
